...
clean_column_names <- function(v_char, remove_digits = FALSE) {
	v_char_original <- v_char
  	v_char_clean <- 
  	  ...
  	  # For complab_migpol avoid double mentioning of dataset in long_tag
  	  gsub("imisem_", "", ., fixed = TRUE) %>%
  	  gsub("demig_", "", ., fixed = TRUE) %>%
  	  gsub("mipex_", "", ., fixed = TRUE) %>%
  	  gsub("globalcit_", "", ., fixed = TRUE) %>%
  	  gsub("impic_", "", ., fixed = TRUE) %>%
...
#' @export
check_multiple_classes <- function(df) {
out <- sapply(df, function(col) length(unique(class(col))) > 1)
names(out[out])
}

#' @export
remove_idate_class <- function(df) {
for (col in names(df)) {
  if (inherits(df[[col]], "IDate")) {
