#' Translation to u_hdata_cabinet_date
#'
#' Translates compatible dataset to primary unit hdata_cabinet_date
#' 
#' @param df a data.frame.
#' @return data.frame translated to hdata_cabinet_date.
#' @examples 
#' to_u_hdata_cabinet_date(df)
#' 
#' @section Note: Unit tables are not passed as function arguments, but are automatically loaded from the \code{ROOT} directory
#'
#' @export
to_u_hdata_cabinet_date <- function(df, ...) UseMethod("to_u_hdata_cabinet_date")

#' @export
to_u_hdata_cabinet_date.default <- function(df, ...) {
  stop("We do not have a method for data from this input unit to u_hdata_cabinet_date!")
}

#' @export
to_u_hdata_cabinet_date.u_hdata_country_year <- function(df) {
  ...
  df$u_hdata_cabinet_date_out_year <- df$u_hdata_country_year_year
  df$u_hdata_cabinet_date_country<- df$u_hdata_country_year_country
  ...
  unit_table_end <- read_unit_table("u_hdata_cabinet_date")
  
  out <- left_join(unit_table_end, df, 
                   by = c("u_hdata_cabinet_date_country", "u_hdata_cabinet_date_out_year"))
...
# Drop all variables without a single match in the end output unit
  ...
  #set class
  class(out) <- c("u_hdata_cabinet_date", old_ds_class, "data.frame")
...
#' @export
to_u_hdata_cabinet_date.u_complab_country_year <- function(df) {
  ...
  #duplicate unit columns
  df$u_hdata_cabinet_date_out_year <- df$u_complab_country_year_year
  df$u_hdata_cabinet_date_country<- df$u_complab_country_year_country
...
#' @export
to_u_hdata_cabinet_date.u_vdem_country_year <- function(df) {
  ...
  #duplicate unit columns
  df$u_hdata_cabinet_date_out_year <- df$u_vdem_country_year_year
  df$u_hdata_cabinet_date_country<- df$u_vdem_country_year_country
  ...
  unit_table_end %<>% 
  mutate(u_hdata_cabinet_date_country  = case_when(
      u_hdata_cabinet_date_country == "Turkey" ~ "Türkiye",
    TRUE ~ u_hdata_cabinet_date_country 
    )) 
...
#' @export
to_u_hdata_cabinet_date.u_vdem_country_date <- function(df) {
  ...
  dfa <- read_unit_table("u_hdata_cabinet_date")
  
  # unit  cols for interpolation, using date_out, this can be adjusted.
  dfa$u_vdem_country_date_date <- dfa$u_hdata_cabinet_date_date_out
  dfa$u_vdem_country_date_country_name <- dfa$u_hdata_cabinet_date_country
  ...
  #unit cols for join
  df$u_hdata_cabinet_date_country <- df$u_vdem_country_date_country_name
  df$u_hdata_cabinet_date_date_out <- df$u_vdem_country_date_date
  ...
  unit_table_end %<>% 
  mutate(
    u_hdata_cabinet_date_country  = case_when(
      u_hdata_cabinet_date_country == "Turkey" ~ "Türkiye",
    TRUE ~ u_hdata_cabinet_date_country 
    ))  
  ...
  #join end unit table
  out <- left_join(unit_table_end, df, 
                   by = c("u_hdata_cabinet_date_country", "u_hdata_cabinet_date_date_out")) %>% 
...
#' @export
to_u_hdata_cabinet_date.u_hdata_minister_date <- function(df) {
  ...
  #create unit cols
  df$u_hdata_cabinet_date_country <- df$u_hdata_minister_date_country
  df$u_hdata_cabinet_date_date_in <- df$u_hdata_minister_date_date
  ...
  out <- left_join(unit_table_end, df, 
                   by = c("u_hdata_cabinet_date_country", "u_hdata_cabinet_date_date_in"))
...
## We leave out the qog translation as the qog CY unit starts in 1945 and hdata_cabinet_date ends in 1945
## We leave out the repdem translation as the hcab dataset collects data on cabinets that preceed the
## repdem data collection
## We leave out the ucdp translation as the ucdp CY unit starts in 1946/1989 and hdata_cabinet_date ends in 1945
