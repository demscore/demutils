  ...
  unit_table_end %<>% 
    dplyr::mutate(
      u_vdem_country_year_country = case_when(
        ...
        u_qog_country_year_country == "Turkey" ~ "Türkiye",
...
to_u_qog_country_year.u_complab_country_year <- function(df, ...) {
  ...
  no_duplicates(unit_table_end, c("u_qog_country_year_ccodealp", "u_qog_country_year_year"))
  ...
  # Common names:
  common_names <- compare_country_names(unit_table_start, unit_table_end, 
                                        "u_complab_country_year_country_code", "u_qog_country_year_ccodealp")$common
  ...
  unit_table_end %<>%
    dplyr::mutate(u_complab_country_year_country_code = case_when(
      u_qog_country_year_ccodealp == 'CSK' & 
        u_qog_country_year_year <= 1992 ~ 'CZE',
  ...
  unit_table_end %<>% 
    dplyr::mutate(
      u_qog_region_year_region_name = case_when(
        u_qog_country_year_country == "Czechia" & u_qog_country_year_year >= 1993 ~ "Czech Republic",
...
#'@export
to_u_qog_country_year.u_cses_respondent<- function(df) {
  ...
  out <- dplyr::left_join(unit_table_end, 
                          df, 
                          by = c("u_qog_country_year_ccodealp" = "u_cses_respondent_country_code", 
