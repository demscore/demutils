#' Translation to u_qog_resp_eqi_21
#'
#' Translates compatible dataset to primary unit u_qog_resp_eqi_21
#' 
#' @param df a data.frame.
#' @return data.frame translated to  u_qog_resp_eqi_21
#' @examples 
#' to_u_qog_resp_eqi_21(df)
#' 
#' @section Note: Unit tables are not passed as function arguments, but are automatically loaded from the \code{ROOT} directory
#'
#' @export
to_u_qog_resp_eqi_21 <- function(df, ...) UseMethod("to_u_qog_resp_eqi_21")
# This function can find the first unit table?
# This function can find the second unit table?

#' @export
to_u_qog_resp_eqi_21.default <- function(df, ...) {
  stop("We do not have a method for data from this input unit to u_qog_resp_eqi_21!")
}

#' @export
to_u_qog_resp_eqi_21.u_qog_country_year <- function(df) {
  
  #save old class
  old_ds_class <- class(df)[2]
  
  unit_table_start <- read_unit_table("u_qog_country_year")
  unit_table_end <- read_unit_table("u_qog_resp_eqi_21")
  
  # Transform unit tables
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_qog_country_year_country = u_qog_resp_eqi_21_country,
                  u_qog_country_year_year = 2021)
  
  # Common names:
  common_names <- compare_country_names(unit_table_start, unit_table_end, 
                                        "u_qog_country_year_country", "u_qog_resp_eqi_21_country")$common
  ...
  unit_table_end %<>% 
    dplyr::mutate(
      u_qog_country_year_country = case_when(
        u_qog_resp_eqi_21_country == "Netherlands" ~ "Netherlands (the)",
        u_qog_resp_eqi_21_country == "Czech Republic" ~ "Czechia",
...
# Drop all variables without a single match in the end output unit
  ...
  class(out) <- c("u_qog_resp_eqi_21", old_ds_class, "data.frame")
  
  return(out)
}

#' @export
to_u_qog_resp_eqi_21.u_qog_region_year <- function(df) {
  ...
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_qog_region_year_region_name = u_qog_resp_eqi_21_country,
                  u_qog_region_year_year = 2021)
  
  # Common names:
  common_names <- compare_country_names(unit_table_start, unit_table_end, 
                                        "u_qog_region_year_region_name", "u_qog_resp_eqi_21_country")$common
