...
download_refs <- function(
	...
	projects <- DBI::dbGetQuery(db, "SELECT * FROM projects WHERE active IS TRUE;") %>% dplyr::select(-active) 
	...
	variable_search <- DBI::dbGetQuery(db, "SELECT * FROM variable_search;")
	location_lookup <- DBI::dbGetQuery(db, "SELECT * FROM location_lookup;")
	variable_unit_combinations <- DBI::dbGetQuery(db, "SELECT * FROM variable_unit_combinations;")
	...
	vbase::write_file(country_years, file.path(OUTDIR, "country_years.rds"))
	vbase::write_file(country_dates, file.path(OUTDIR, "country_dates.rds"))
	vbase::write_file(conflict_location_years, file.path(OUTDIR, "conflict_location_years.rds"))
	vbase::write_file(region_years, file.path(OUTDIR, "region_years.rds"))
	vbase::write_file(plot_regions, file.path(OUTDIR, "plot_regions.rds"))
	vbase::write_file(variable_search, file.path(OUTDIR, "variable_search.rds"))
	vbase::write_file(location_lookup, file.path(OUTDIR, "location_lookup.rds"))
	vbase::write_file(variable_unit_combinations, file.path(OUTDIR, "variable_unit_combinations.rds"))
...
copy_static_refs <- function(
	...
	file.copy(file.path(REFS_DIR, "demscorecodebook", "genderfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "genderfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "genderfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "genderfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "genderfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "genderfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "environmentfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "environmentfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "pertiesfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "partiesfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "educationfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "educationfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "unemploymentfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "unemploymentfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "mediafrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "mediafrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REFS_DIR, "demscorecodebook", "migrationfrontpage.pdf"),
			  file.path(REFS_OUT_DIR, "migrationfrontpage.pdf"), overwrite = TRUE)
	return(invisible(NULL))
}
...
prepare_refs <- function(
	...
	country_years <- vbase::read_file(file.path(INDIR, "country_years.rds"))
	country_dates <- vbase::read_file(file.path(INDIR, "country_dates.rds"))
	conflict_location_years <- vbase::read_file(file.path(INDIR, "conflict_location_years.rds"))
	region_years <- vbase::read_file(file.path(INDIR, "region_years.rds"))
	plot_regions <- vbase::read_file(file.path(INDIR, "plot_regions.rds"))
	variable_search <- vbase::read_file(file.path(INDIR, "variable_search.rds"))
	location_lookup <- vbase::read_file(file.path(INDIR, "location_lookup.rds"))
	variable_unit_combinations <- 
		vbase::read_file(file.path(INDIR, "variable_unit_combinations.rds"))
	...
	vbase::write_file(variable_search, file.path(OUTDIR, "variable_search.rds"))
	vbase::write_file(location_lookup, file.path(OUTDIR, "location_lookup.rds"))
	vbase::write_file(variable_unit_combinations, 
		file.path(OUTDIR, "variable_unit_combinations.rds"))
	return(invisible(NULL))
}
