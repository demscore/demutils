#' Translation to u_ucdp_par_event
#'
#' Translates compatible dataset to primary unit ucdp_par_event
#' 
#' @param df a data.frame.
#' @return data.frame translated to cucdp_par_event
#' @examples 
#' to_u_ucdp_par_event(df)
#' 
#' @section Note: Unit tables are not passed as function arguments, but are automatically loaded from the \code{ROOT} directory
#'
#' @export
to_u_ucdp_par_event <- function(df, ...) UseMethod("to_u_ucdp_par_event")
# This function can find the first unit table?
# This function can find the second unit table?

#' @export
to_u_ucdp_par_event.default <- function(df, ...) {
  stop("We do not have a method for data from this input unit to u_ucdp_par_event!")
}


#' @export
to_u_ucdp_par_event.u_ucdp_dyad_year <- function(df) {
  
  #save old class
  old_ds_class <- class(df)[2]
  
  unit_table_start <- read_unit_table("u_ucdp_dyad_year")
  unit_table_end <- read_unit_table("u_ucdp_par_event")
  
  
  # Transform unit tables
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_ucdp_dyad_year_dyad_id = u_ucdp_par_event_dyad_id,
                  u_ucdp_dyad_year_year = u_ucdp_par_event_year)
  ...
  class(out) <- c("u_ucdp_par_event", old_ds_class, "data.frame")
  
  return(out)
}

#' @export
to_u_ucdp_par_event.u_ucdp_country_year <- function(df) {
  ...
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_ucdp_orgv_country_year_country_id_cy = u_ucdp_par_event_gwno_a,
                  u_ucdp_country_year_year_cy = u_ucdp_par_event_year)
  
  
  stopifnot(nrow(unit_end_original) == nrow(unit_table_end))
  
  int_df <- bind_cols(unit_table_start, df) 
  int_df$u_ucdp_orgv_country_year_country_id_cy <- as.character(int_df$u_ucdp_orgv_country_year_country_id_cy)
  
  # Output data
  out <- dplyr::left_join(unit_table_end, int_df,
                          by = c("u_ucdp_orgv_country_year_country_id_cy", 
                                 "u_ucdp_country_year_year_cy"))
...
#' @export
to_u_ucdp_par_event.u_qog_country_year <- function(df) {
  ...
  #duplicate unit cols
  df$u_ucdp_par_event_country <- df$u_qog_country_year_country
  df$u_ucdp_par_event_year <- df$u_qog_country_year_year
  
  #adjust country names
  df %<>% 
    dplyr::mutate(u_ucdp_par_event_country = case_when(
  ...
  out <- left_join(unit_table_end, df, 
                   by = c("u_ucdp_par_event_country", "u_ucdp_par_event_year"))
...
#' @export
to_u_ucdp_par_event.u_vdem_country_year <- function(df) {
  ...
  #duplicate unit cols
  df$u_ucdp_par_event_country <- df$u_vdem_country_year_country
  df$u_ucdp_par_event_year <- df$u_vdem_country_year_year
...
#' @export
to_u_ucdp_par_event.u_complab_country_year <- function(df) {
  ...
  #duplicate unit cols
  df$u_ucdp_par_event_country <- df$u_complab_country_year_country
  df$u_ucdp_par_event_year <- df$u_complab_country_year_year
...
#' @export
to_u_ucdp_par_event.u_hdata_country_year <- function(df) {
  ...
  #duplicate unit cols
  df$u_ucdp_par_event_country <- df$u_hdata_country_year_country
  df$u_ucdp_par_event_year <- df$u_hdata_country_year_year
...
#' @export
to_u_ucdp_par_event.u_repdem_cabinet_date <- function(df) {
  ...
  #duplicate unit cols
  df$u_ucdp_par_event_country <- df$u_repdem_cabinet_date_country
  df$u_ucdp_par_event_year <- df$u_repdem_cabinet_date_year
  
  #adjust country names
  df %<>% 
    dplyr::mutate(
      u_ucdp_par_event_country = case_when(
