...
to_u_hdata_minister_date.u_vdem_country_date <- function(df) {
  ...
  df %<>% 
    dplyr::mutate(u_vdem_country_date_country_name = case_when(
      u_vdem_country_date_country_name == "Türkiye" ~ "Ottoman Empire/Turkey",
...
# Drop all variables without a single match in the end output unit
    drop_cols <- names(out)[sapply(out, function(col) all(col == -11111))] #|col == "-11111" returns error
  if (length(drop_cols) > 0) {
    out <- out[, !names(out) %in% drop_cols, drop = FALSE]
  } else {
    out <- out
...
#' @export
to_u_hdata_minister_date.u_hdata_cabinet_date <- function(df) {
  ...
  # Because hdata cabinet date represents an interval during which a cabinet is in power,
  # I am duplicating the data across -all individual dates- within this interval in order to then match
  # on other date units. 
  ...
  #duplicate unit cols
  df$u_hdata_minister_date_country <- df$u_hdata_cabinet_date_country
  df$u_hdata_minister_date_date_in <- df$u_hdata_cabinet_date_date
