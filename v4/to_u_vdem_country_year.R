  ...
  unit_table_end %<>% 
    dplyr::mutate(
      u_qog_country_year_country = case_when(
        ...
        u_vdem_country_year_country == "Sudan" & 
          u_vdem_country_year_year >= 2012 ~ "Sudan (the)",
        ...
        u_vdem_country_year_country == "Syria" ~ "Syrian Arab Republic (the)",
        u_vdem_country_year_country == "Taiwan" ~ "Taiwan (Province of China)",
        u_vdem_country_year_country == "Türkiye" ~ "Turkey",
...
to_u_vdem_country_year.u_complab_country_year <- function(df, ...) {
  ...
  no_duplicates(unit_table_end, c("u_vdem_country_year_country_text_id", "u_vdem_country_year_year"))
  
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_complab_country_year_country_code = NA_character_,
                  u_complab_country_year_year = u_vdem_country_year_year)
  
  # Common names:
  common_names <- compare_country_names(unit_table_start, unit_table_end, 
                                        "u_complab_country_year_country_code", "u_vdem_country_year_country_text_id")$common
  ...
  unit_table_end %<>% 
    dplyr::mutate(
      u_complab_country_year_country_code =
        ifelse(u_vdem_country_year_country_text_id %in% common_names,
               u_vdem_country_year_country_text_id, NA))
...
to_u_vdem_country_year.u_vdem_party_country_year <- function(df) {
  ...
  # PSDI
  psdi <- df %>% create_psdi()
  psdi_low <- df %>% create_psdi_codelow()
  psdi_high <- df %>% create_psdi_codehigh()
  
  # PSLRI
  pslri <- df %>% create_pslri()
  pslri_low <- df %>% create_pslri_codelow()
  pslri_high <- df %>% create_pslri_codehigh()
  
  # PSREI
  psrei <- df %>% create_psrei()
  psrei_low <- df %>% create_psrei_codelow()
  psrei_high <- df %>% create_psrei_codehigh()
  
  # PSEXI
  psexi <- df %>% create_psexi()
  psexi_low <- df %>% create_psexi_codelow()
  psexi_high <- df %>% create_psexi_codehigh()
  
  df <- bind_cols(psdi, psdi_low, psdi_high,
                  psrei, psrei_low, psrei_high,
                  psexi, psexi_low, psexi_high)
  
  df_lr <- bind_cols(pslri, pslri_low, pslri_high)
  
  df <- left_join(df, df_lr, by = c("vdem_vparty_country_name", "vdem_vparty_year"))
  
  
  df$u_vdem_party_country_year_country_name <- df$vdem_vparty_country_name
  df$u_vdem_party_country_year_year <- df$vdem_vparty_year
  ...
  out %<>% select(-vdem_vparty_year, -vdem_vparty_country_name) %>%
    vbase::round_df(., 3)
...
#' @export
to_u_vdem_country_year.u_hdata_cabinet_date <- function(df) {
  ...
  #duplicate unit cols
  df$u_vdem_country_year_country <- df$u_hdata_country_year_country
  df$u_vdem_country_year_year <- as.integer(df$u_hdata_country_year_year)
...
#' @export
to_u_vdem_country_year.u_cses_respondent <- function(df) {
#aggregate from cab date to country date to country year
  df %<>% 
    aggregate_cses(df)
  ...
  out <- dplyr::left_join(unit_table_end, 
                          df, 
                          by = c("u_vdem_country_year_country_id" = "u_cses_respondent_vdem_country_code", 
