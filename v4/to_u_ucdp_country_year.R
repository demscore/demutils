#' Translation to u_ucdp_country_year
#'
...
#' @examples 
#' to_u_ucdp_country_year(df)
#' 
#' @section Note: Unit tables are not passed as function arguments, but are automatically loaded from the \code{ROOT} directory
#'
#' @export
to_u_ucdp_country_year <- function(df, ...) UseMethod("to_u_ucdp_country_year")

#' @export
to_u_ucdp_country_year.default <- function(df, ...) {
  stop("We do not have a method for data from this input unit to u_ucdp_country_year!")
}

#' @export
to_u_ucdp_country_year.u_vdem_country_year <- function(df) {
  ...
  unit_table_end <- read_unit_table("u_ucdp_country_year")
  
  unit_table_end$u_ucdp_country_year_gwno_a <- 
    as.numeric(unit_table_end$u_ucdp_country_year_gwno_a)
  ...
  df %<>% 
    mutate(
      u_vdem_country_year_cowcode = case_when(
        ...
        TRUE ~ as.numeric(u_vdem_country_year_cowcode)
      ))
  
  out <- left_join(unit_table_end, df, 
                   by = c("u_ucdp_country_year_year" = "u_vdem_country_year_year", 
                          "u_ucdp_country_year_gwno_a" = "u_vdem_country_year_cowcode"))
...
# Drop all variables without a single match in the end output unit
  ...
  #set class
  class(out) <- c("u_ucdp_country_year", old_ds_class, "data.frame")
...
#' @export
to_u_ucdp_country_year.u_qog_country_year <- function(df) {
  ...
  df %<>% 
    mutate(
      u_qog_country_year_ccodecow = case_when(
        ...
        TRUE ~ as.numeric(u_qog_country_year_ccodecow)
      ))
  
  out <- left_join(unit_table_end, df, 
                   by = c("u_ucdp_country_year_year" = "u_qog_country_year_year", 
                          "u_ucdp_country_year_gwno_a" = "u_qog_country_year_ccodecow"))
...
#' @export
to_u_ucdp_country_year.u_hdata_country_year <- function(df) {
  ...
  out <- dplyr::left_join(unit_table_end, df, 
                   by = c("u_ucdp_country_year_year" = "u_hdata_country_year_year", 
                          "u_ucdp_country_year_gwno_a" = "u_hdata_country_year_cowcode"))
...
#' @export
to_u_ucdp_country_year.u_complab_country_year <- function(df) {
  ...
  out <- dplyr::left_join(unit_table_end, df, 
                   by = c("u_ucdp_country_year_year" = "u_complab_country_year_year", 
                          "u_ucdp_country_year_name" = "u_complab_country_year_country"))
...
#' @export
to_u_ucdp_country_year.u_repdem_cabinet_date <- function(df) {
  ...
  out <- dplyr::left_join(unit_table_end, df, 
                   by = c("u_ucdp_country_year_year" = "u_repdem_country_year_year", 
                          "u_ucdp_country_year_name" = "u_repdem_country_year_country"))
...
#' @export
to_u_ucdp_country_year.u_ucdp_orgv_country_year <- function(df) {
  ...
  unit_table_end <- read_unit_table("u_ucdp_country_year")
  unit_table_end$u_ucdp_country_year_gwno_a <- 
    as.integer(unit_table_end$u_ucdp_country_year_gwno_a)
  
  df %<>% bind_cols(unit_table_start)
  
  
  out <- left_join(unit_table_end, df, 
                   by = c("u_ucdp_country_year_year" = "u_ucdp_orgv_country_year_year_cy", 
                          "u_ucdp_country_year_gwno_a" = "u_ucdp_orgv_country_year_country_id_cy"))
