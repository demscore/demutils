...
codebook_load_data <- function(args, POSTGRES_TABLES_DIR, LOCAL) {
	...
	if (isTRUE(!LOCAL)) {
		...
		cb_section <- DBI::pgGetQuery(db, "SELECT * FROM cb_section;") 
		datasets <- DBI::pgGetQuery(db, "SELECT * FROM datasets;") 
		units <- DBI::pgGetQuery(db, "SELECT * FROM units;") 
		projects <- DBI::pgGetQuery(db, "SELECT * FROM projects;") 
		unit_variables <- DBI::pgGetQuery(db, "SELECT * FROM unit_variables;") 
		thematic_datasets <- DBI::pgGetQuery(db, "SELECT * FROM thematic_datasets;") 
		thematic_datasets_variables <- DBI::pgGetQuery(db, "SELECT * FROM thematic_datasets_variables;") 
		merge_scores <- DBI::pgGetQuery(db, "SELECT * FROM merge_scores;") 	
		variables <- DBI::pgGetQuery(db, "SELECT * FROM variables;") 
...
codebook_prepare_data <- function(ll, args, include_unit_variables = TRUE) {
  ...
  variables <- ll$variables
	...
	mode <- dplyr::case_when(
		...
		!is.null(args$thematic) & length(args$variables) > 0L ~ "thematic",
	...
	if(mode == "thematic") {
	    ...
	    pdf_file <- switch(THEME,
	                       "gender" = "genderfrontpage",
	                       "environment" = "environmentfrontpage",
	                       "parties_and_elections" = "partiesfrontpage",
	                       "media" = "mediafrontpage",
	                       "migration" = "migrationfrontpage",
	                       "education" = "educationfrontpage",
	                       "unemployment" = "unemploymentfrontpage",
						   "security" = "securityfrontpage",
						   "corruption" = "corruptionfrontpage",
	                       stop("Unknown theme!"))
	    
	    intro <- gsub("\\\\includepdf\\[pages=\\{1\\}\\]\\{\\.\\/codebookfrontpage\\}", 
	                  paste0("\\\\includepdf\\[pages=\\{1\\}\\]\\{.\\/", pdf_file, "\\}"), intro)
	...
	if (mode == "project") {
		...
		pdf_file <- switch(PROJECT,
			"hdata" = "hdatafrontpage",
			"qog" = "qogfrontpage",
			"vdem" = "vdemfrontpage",
			"ucdp" = "ucdpfrontpage",
			"complab" = "complabfrontpage",
			"repdem" = "repdemfrontpage",
	...
	if (mode == "dataset") {
		...
		if (grepl("^hdata", DATASET)) {
			pdf_file <- "hdatafrontpage"
			} else if (grepl("^qog", DATASET)) {
			pdf_file <- "qogfrontpage"
			} else if (grepl("^vdem", DATASET)) {
			pdf_file <- "vdemfrontpage"
			} else if (grepl("^ucdp", DATASET)) {
			pdf_file <- "ucdpfrontpage"
			} else if (grepl("^views", DATASET)) {
			pdf_file <- "ucdpfrontpage"
			} else if (grepl("^complab", DATASET)) {
			pdf_file <- "complabfrontpage"
			} else if (grepl("^repdem", DATASET)) {
			pdf_file <- "repdemfrontpage"
			} else {
	...
	# download_row_idx is hardcoded for the position of the row that is to be augmented
	download_row_idx <- grep(pattern = "\\\\subsection\\{Download ID\\}", x = intro) + 3
	stopifnot(`download_row_idx is null` = !is.null(download_row_idx))
	stopifnot(`download_row_idx is not numeric` = is.numeric(download_row_idx))
	intro[download_row_idx] <- paste0(intro[download_row_idx], DOWNLOAD_ID)
...
codebook_build_pdf_from_tex <- function(ll, args, REF_STATIC_DIR) {
	...
	file.copy(file.path(REF_STATIC_DIR, "codebookfrontpage.pdf"), 
	          file.path(temp_dir, "/codebookfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "genderfrontpage.pdf"),
			  file.path(temp_dir, "/genderfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "environmentfrontpage.pdf"),
			  file.path(temp_dir, "/environmentfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "partiesfrontpage.pdf"),
			  file.path(temp_dir, "/partiesfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "mediafrontpage.pdf"),
			  file.path(temp_dir, "/mediafrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "migrationfrontpage.pdf"),
			  file.path(temp_dir, "/migrationfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "educationfrontpage.pdf"),
			  file.path(temp_dir, "/educationfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "unemploymentfrontpage.pdf"),
			  file.path(temp_dir, "/unemploymentfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "securityfrontpage.pdf"),
			  file.path(temp_dir, "/securityfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "corruptionfrontpage.pdf"),
			  file.path(temp_dir, "/corruptionfrontpage.pdf"), overwrite = TRUE)
	# Copy project frontpages
	file.copy(file.path(REF_STATIC_DIR, "complabfrontpage.pdf"),
			  file.path(temp_dir, "/complabfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "hdatafrontpage.pdf"),
			  file.path(temp_dir, "/hdatafrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "qogfrontpage.pdf"),
			  file.path(temp_dir, "/qogfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "repdemfrontpage.pdf"),
			  file.path(temp_dir, "/repdemfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "ucdpfrontpage.pdf"),
			  file.path(temp_dir, "/ucdpfrontpage.pdf"), overwrite = TRUE)
	file.copy(file.path(REF_STATIC_DIR, "vdemfrontpage.pdf"),
			  file.path(temp_dir, "/vdemfrontpage.pdf"), overwrite = TRUE)
    
	
	OUTFILE <- args$outfile
	
	# Create pdf from tex file and bibtex file and codebook frontpage
	#.call = "latexmk -cd -f -e '$max_repeat=4' -silent -jobname=codebook -pdf" %^% 
	...
	#previousWD <- getwd()
	#setwd(temp_dir)
	#system("sed -i 's/NA//g' refs.bib")
	#system("pdflatex -interaction=nonstopmode codebook.tex \ biber codebook.tex \ pdflatex codebook.tex \ pdflatex codebook.tex; cp #codebook.pdf" %^% " " %^% OUTFILE, intern = TRUE,
	...
	previousWD <- getwd()
	setwd(temp_dir)
	system("sed -i '/^\\\\harvarditem{aaa}/d; /^aaa/d; /^\\\\harvarditem{.}{n\\.d}/d; /^\\./d' codebook.bbl")
	system("sed -i '/^$/d' codebook.bbl")
	system("sed -i 's/NA//g' refs.bib")
	setwd(previousWD)
	...
	# The codebook generation creates false TOC when codebooks are created in a loop, bco 
	# not all latex files are updated in the temp_dir in a loop. Hence, we unlink the tempdir
	# unless we choose the mode "variables".
	mode <- dplyr::case_when(
		length(args$variables) > 0L ~ "variables",
		!is.null(args$project) ~ "project",
		!is.null(args$dataset) ~ "dataset",
...
create_thematic_dataset_codebook <- function(args, REF_STATIC_DIR, 
	...
	args %>%
		...
		codebook_prepare_data(., args, include_unit_variables = TRUE) %>%
		codebook_build_pdf_from_tex(., args, REF_STATIC_DIR)
}
