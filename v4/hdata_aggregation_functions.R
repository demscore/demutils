...
#' Aggregation hdata_country_date_to_country_year_ministers
#' 
...
#' @export
#' 
hdata_country_date_to_country_year_ministers <- function(df) {
...
hdata_minister_date_to_country_year <- function(df) {
  ...
  country_year <- demutils::hdata_country_date_to_country_year_ministers(country_date)
...
#' 
#' Aggregates from hdata_cabinet_date to hdata_country_date
#' 
#' @param df a data.frame
#' @return data.frame aggregated to hdata_country_date.
#' @examples 
#' hdata_cabinet_date_to_country_date(df)
#'
#' @export
hdata_cabinet_date_to_country_date <- function(df) {
  
  #bind in unit cols
  unit_table_start <- read_unit_table("u_hdata_cabinet_date")
  ...
  hdata_overlap_adjust <- function(df){
    ...
    df %<>% 
      dplyr::group_by(u_hdata_cabinet_date_country) %>% 
      dplyr::arrange(u_hdata_cabinet_date_date_in, .by_group = TRUE) %>%
      #if there is an overlap, subtract 1 day from date_out
      dplyr::mutate(u_hdata_cabinet_date_date_out = case_when(
        u_hdata_cabinet_date_date_out >= lead(u_hdata_cabinet_date_date_in) &
          u_hdata_cabinet_date_country == lead(u_hdata_cabinet_date_country) ~
          lead(u_hdata_cabinet_date_date_in) -1,
        TRUE ~ as.Date(u_hdata_cabinet_date_date_out)
      )) %>%
      dplyr::ungroup(.)
  ...
  hdata_newdate_cols <- function(df) {
    ...
    date_out_df <- df <- filter(df, u_hdata_cabinet_date_date_in != u_hdata_cabinet_date_date_out)
    
    date_out_df$u_hdata_cabinet_date_newdate <- 
      date_out_df$u_hdata_cabinet_date_date_out
    
    df$u_hdata_cabinet_date_newdate <- 
      df$u_hdata_cabinet_date_date_in
    
    df %<>% 
      rbind(date_out_df) %>%
      dplyr::arrange(u_hdata_cabinet_date_country, u_hdata_cabinet_date_date_in)
  ...
  df %<>% 
    dplyr::rename(newdate = u_hdata_cabinet_date_newdate)
  
  stopifnot(no_duplicates(df, c("u_hdata_cabinet_date_country", 
                                "newdate")))
  return(df)
...
#' @return data.frame aggregated to hdata_country_year.
#' @examples 
#' hdata_country_date_to_country_year_cabinets(df)
#'
#' @export
hdata_country_date_to_country_year_cabinets <- function(df) {
  ...
  country_years <- 
    data.frame(merge((unique(df$u_hdata_cabinet_date_country)), 
                     (min(df$u_hdata_cabinet_date_in_year):max(df$u_hdata_cabinet_date_out_year)))) 
  
  colnames(country_years) <- 
    c('u_hdata_cabinet_date_country', 'u_hdata_cabinet_date_year')
  
  
  # Creating df with all country years represented (cabinets spanning multiple years duplicated), and arranging.
  all_cabs_all_years <-
    dplyr::inner_join(country_years, df, by = c("u_hdata_cabinet_date_country")) %>%
    dplyr::filter(u_hdata_cabinet_date_year >= u_hdata_cabinet_date_in_year, 
                  u_hdata_cabinet_date_year <= u_hdata_cabinet_date_out_year) %>%
    dplyr::arrange(u_hdata_cabinet_date_country, 
                   u_hdata_cabinet_date_year)
  ...
  # day_count_dataframe creates a df with days per year.
  day_count_dataframe <- function(k) gen_all_days(df$u_hdata_cabinet_date_date_in[k],
                                                  df$u_hdata_cabinet_date_date_out[k])
  ...
  country_year_subset <- 
    ...
    dplyr::select(u_hdata_cabinet_date_year_check = V1, everything()) %>% 
    dplyr::group_by(u_hdata_cabinet_date_country, 
                    u_hdata_cabinet_date_year_check) %>%
    dplyr::slice_max(as.integer(day_count), n = 1) %>%
    dplyr::select(-u_hdata_cabinet_date_year) %>%
    dplyr::rename(u_hdata_cabinet_date_year = u_hdata_cabinet_date_year_check) %>%
    dplyr::ungroup(.) %>%
    dplyr::distinct(u_hdata_cabinet_date_country, u_hdata_cabinet_date_year, 
                    u_hdata_cabinet_date_cab_name, 
                    u_hdata_cabinet_date_date_in, u_hdata_cabinet_date_date_out, 
                    .keep_all = TRUE)
  
  #check <- country_year_subset %>% select(u_hdata_cabinet_date_country, u_hdata_cabinet_date_year_check, 
  #u_hdata_cabinet_date_cab_name, 
  #u_hdata_cabinet_date_date_in, u_hdata_cabinet_date_date_out,
  #day_count) %>% distinct(.)
  ...
  #create country year unit columns
  country_year_subset$u_hdata_country_year_country <- 
    country_year_subset$u_hdata_cabinet_date_country
  
  country_year_subset$u_hdata_country_year_year <- 
    country_year_subset$u_hdata_cabinet_date_year
  ...
  stopifnot(no_duplicates(country_year_subset,
                          c("u_hdata_country_year_country", "u_hdata_country_year_year")))
  
  return(country_year_subset)
  
}

#' hdata_cabinet_date_to_country_year
#' 
...
#' @return data.frame aggregated to hdata_country_year.
#' @examples 
#' hdata_cabinet_date_to_country_year(df)
#'
#' @export
hdata_cabinet_date_to_country_year <- function(df) {
  
  country_date <- demutils::hdata_cabinet_date_to_country_date(df)
  country_year <- demutils::hdata_country_date_to_country_year_cabinets(country_date)
