#' Translation to u_qog_resp_eqi_perc_17
#'
...
#' @examples 
#' to_u_qog_resp_eqi_perc_17(df)
#' 
#' @section Note: Unit tables are not passed as function arguments, but are automatically loaded from the \code{ROOT} directory
#'
#' @export
to_u_qog_resp_eqi_perc_17 <- function(df, ...) UseMethod("to_u_qog_resp_eqi_perc_17")
# This function can find the first unit table?
# This function can find the second unit table?

#' @export
to_u_qog_resp_eqi_perc_17.default <- function(df, ...) {
  stop("We do not have a method for data from this input unit to u_qog_resp_eqi_perc_17!")
}

#' @export
to_u_qog_resp_eqi_perc_17.u_qog_country_year <- function(df) {
  
  #save old class
  old_ds_class <- class(df)[2]
  
  unit_table_start <- read_unit_table("u_qog_country_year")
  unit_table_end <- read_unit_table("u_qog_resp_eqi_perc_17")
  
  # Transform unit tables
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_qog_country_year_country = u_qog_resp_eqi_perc_17_country,
                  u_qog_country_year_year = 2017)
  
  # Common names:
  common_names <- compare_country_names(unit_table_start, unit_table_end, 
                                        "u_qog_country_year_country", "u_qog_resp_eqi_perc_17_country")$common
  ...
  unit_table_end %<>% 
    dplyr::mutate(
      u_qog_country_year_country = case_when(
        u_qog_resp_eqi_perc_17_country == "Netherlands" ~ "Netherlands (the)",
...
# Drop all variables without a single match in the end output unit
  ...
  class(out) <- c("u_qog_resp_eqi_perc_17", old_ds_class, "data.frame")
  
  return(out)
}

#' @export
to_u_qog_resp_eqi_perc_17.u_qog_region_year <- function(df) {
  ...
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_qog_region_year_region_name = u_qog_resp_eqi_perc_17_country,
                  u_qog_region_year_year = 2017)
  
  # Common names:
  common_names <- compare_country_names(unit_table_start, unit_table_end, 
                                        "u_qog_region_year_region_name", "u_qog_resp_eqi_perc_17_country")$common
