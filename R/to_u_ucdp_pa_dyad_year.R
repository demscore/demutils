#' Translation to u_ucdp_pa_dyad_year
#'
#' Translates compatible dataset to primary unit ucdp_pa_dyad_year
#' 
#' @param df a data.frame.
#' @return data.frame translated to cucdp_pa_dyad_year
#' @examples 
#' to_u_ucdp_pa_dyad_year(df)
#' 
#' @section Note: Unit tables are not passed as function arguments, but are automatically loaded from the \code{ROOT} directory
#'
#' @export
to_u_ucdp_pa_dyad_year <- function(df, ...) UseMethod("to_u_ucdp_pa_dyad_year")
# This function can find the first unit table?
# This function can find the second unit table?

#' @export
to_u_ucdp_pa_dyad_year.default <- function(df, ...) {
  stop("We do not have a method for data from this input unit to u_ucdp_pa_dyad_year!")
}

#' @export
to_u_ucdp_pa_dyad_year.u_ucdp_dyad_year <- function(df) {
  
  #save old class
  old_ds_class <- class(df)[2]
  
  unit_table_start <- read_unit_table("u_ucdp_dyad_year")
  unit_table_end <- read_unit_table("u_ucdp_pa_dyad_year")
  
  
  # Transform unit tables
  unit_end_original <- unit_table_end
  unit_table_end %<>%
    dplyr::mutate(u_ucdp_dyad_year_dyad_id = as.integer(u_ucdp_pa_dyad_year_dyad_id),
                  u_ucdp_dyad_year_year = as.integer(u_ucdp_pa_dyad_year_year))
  ...
  class(out) <- c("u_ucdp_pa_dyad_year", old_ds_class, "data.frame")
