...
create_methodology <- function(
	...
	# QoG table does not exist anymore	
	# Remove QoG variable table from methodology document:
	#projects$cb_entry[projects$project_short == "qog"] <- 
		#projects$cb_entry[projects$project_short == "qog"] %>%
		# Remove everything after \footnotesize
		#gsub("\\\\footnotesize.*$", "", .) %>%
		# Add \newpage at the end.
		#paste0(., "\\newpage")
	...
	# Copy pdf cover page
	file.copy(file.path(REFDIR, "methodologyfrontpagebeta.pdf"),
		file.path(temp_dir, "methodologyfrontpagebeta.pdf"), overwrite = TRUE)
